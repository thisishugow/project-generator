  ...
  methods: {
    ...
    renderChart(data) {
      ...
      const option = {
        ...
        yAxis: {
          min:this.getMinData(seriesObj) - 0.01*(this.getMinData(seriesObj)),
          axisLabel: {
            show: true,
            color: '#95a4bd',
    ...
    getMinData(seriesObject){
    let dataArr = Object.values(seriesObject).flatMap(item => item.data);
    return Math.min(...dataArr)? Math.min(...dataArr):[...dataArr].sort((a,b)=> a-b)[0]
  },
  },
